...
::: {.example-figure .fragment .current-visible data-fragment-index="2"}
![Example output: Alzheimer's Association funding portfolio by CADRO category — molecular mechanisms (Cat A) take 47% while population studies (Cat D) draw under 5% (Kloske et al., *Alz & Dem* 2025).](assets/alz14354-fig-0002.png)
:::

::: {.example-figure .fragment .current-visible data-fragment-index="3"}
![Example output: Hetionet biomedical knowledge graph schema (left) and compound–disease treatment / non-treatment heatmap derived from typed-path features (right) — the kind of structured representation that powers modern drug-repurposing search (Himmelstein et al., *GigaScience* 2023).](assets/hetionet.jpg)
:::

::: {.example-figure .fragment .current-visible data-fragment-index="5"}
![Example output: VOSviewer citation-network map of journals — clusters and bridging titles surface emerging research fronts long before lagging citation counts move (van Eck & Waltman, VOSviewer).](assets/vosviewer-citation-network.png)
:::
...
/* Per-bullet example figures pinned to the bottom of the slide.
   Multiple .example-figure blocks share the same absolute slot so they
   swap in place as the user advances bullets — this both avoids
   pushing figure N below the viewport (where it'd be invisible under
   the scrollable overflow) and keeps a consistent visual anchor.
   Visibility is controlled by Reveal.js's `current-visible` fragment
   class, set per data-fragment-index. Click any image to open it in
   the lightbox modal. */
...
- **Most open-data signal lives in prose, not metadata.**
- **Composite FAIR scores hide the interesting variance.**
- **Domain and centre effects dominate.**
- **Pipelines need guardrails.**
